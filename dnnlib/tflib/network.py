...
class Network:
    ...
    def __init__(
        self, num_inputs: int, name: str = None, func_name: Any = None, **static_kwargs
    ):
    ...
    def _init_graph(self) -> None:
        ...
        with tfutil.absolute_variable_scope(
            ...
            with tf.control_dependencies(
                ...
                self.input_templates = [
                    tf.placeholder(tf.float32, name="input_" + str(inp_num))
        ...
        self.input_names = [
            inp.name.split("/")[-1][:-2] for inp in self.input_templates
        ]
        ...
        **dynamic_kwargs
        ...
        def unwind_key(obj):
            if isinstance(obj, dict):
                return [
                    (in_key, unwind_key(value)) for in_key, value in sorted(obj.items())
                ]
            if callable(obj):
