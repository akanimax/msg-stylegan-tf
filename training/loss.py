...
def D_logistic_simplegp(
    ...
    if r1_gamma != 0.0:
        with tf.name_scope("R1Penalty"):
            ...
            real_grads = [
                opt.undo_loss_scaling(real_grad)
                for real_grad in fp32(*tf.gradients(real_loss, reals))
            ]
    ...
    if r2_gamma != 0.0:
        with tf.name_scope("R2Penalty"):
            ...
            fake_grads = [
                opt.undo_loss_scaling(fake_grad)
                for fake_grad in fp32(*tf.gradients(fake_loss, [fake_images_out]))
            ]
